...
const DESKTOP_APP_ID = "com.heyjules.desktop";
...
export function resolveDesktopProductName(version: string): string {
  return resolveDesktopUpdateChannel(version) === "nightly"
    ...
    : (desktopPackageJson.productName ?? "HeyJules Desktop");
}
...
export const createBuildConfig = Effect.fn("createBuildConfig")(function* (
  ...
  const buildConfig: Record<string, unknown> = {
    ...
    artifactName: "HeyJules-Desktop-${version}-${arch}.${ext}",
  ...
  if (platform === "linux") {
    buildConfig.linux = {
      target: [target],
      executableName: "heyjules-desktop",
      icon: "icons",
      category: "Development",
      // electron-builder turns these into MimeType=x-scheme-handler/<scheme>;
      // in the .desktop entry (Exec already gets %U), so browsers can hand
      // heyjules:// OAuth callbacks to the app.
      protocols: [
        {
          name: "HeyJules Desktop",
          schemes: ["heyjules", "heyjules-dev"],
      ...
      desktop: {
        entry: {
          StartupWMClass: "heyjules-desktop",
        },
      },
    };
  }
...
const buildDesktopArtifact = Effect.fn("buildDesktopArtifact")(function* (
  ...
  const stagePackageJson: StagePackageJson = {
    ...
    description: "HeyJules Desktop experimental build",
